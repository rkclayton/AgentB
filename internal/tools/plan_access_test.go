...
func TestFileToolsReadAllPlansAndWriteOnlyBoundPlan(t *testing.T) {
	...
	b := &session.Session{ID: "b", Role: "b", Workspace: folder, PlansRoot: plans, LastSeen: map[string]time.Time{}, ToolsEnabled: enabled}
	d := &session.Session{ID: "d", Role: "d", Workspace: folder, PlansRoot: plans, PlanDir: own, LastSeen: map[string]time.Time{}, ToolsEnabled: enabled}
