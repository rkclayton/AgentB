...
| Tool caps | `tools.{read_file,list_dir,grep}`, `shell.{command,timeout_s,max_timeout_s,max_output_lines_head,max_output_lines_tail,file_routing_guard,operator_context,service_account,deny}` (`file_routing_guard` defaults on; `operator_context` and `service_account.enabled` default off) |
