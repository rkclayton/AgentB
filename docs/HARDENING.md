...
- Every existing top-level file and directory in the AgentB application tree receives an explicit service-account write/delete/ownership deny, recursively for directories. This includes the binary, source, UI, prompts, configuration, scripts, `.git`, logs, memory, and local toolchains. Read and execute remain available.
- The selected workspace receives an explicit recursive `Modify` grant. If it is inside the AgentB directory, it must be one direct child so it can be excluded cleanly from the recursive denies.
...
Select **Verify** at any time to detect missing or replaced ACL entries and firewall drift. Reapply after updating, rebuilding, or adding files to AgentB because a newly created or replaced application file may not retain its explicit deny.
...
Run these through AgentB after **Apply + verify**:
