...
- The LocalAppData data root denies the service account every right except traversal. Configuration, the DPAPI credential, logs, retained chats, and memory remain unavailable to it. The one exception is the `plans` child, which receives explicit recursive `Modify` so D-role file tools can use their bound plan folder under the service identity.
- Parent directories receive only the traverse grants needed to reach the Program Files application, LocalAppData plans folder, ProgramData workspace, and configured exchange folder. Plans, workspace, and exchange each receive an explicit recursive `Modify` grant. Application, data, workspace, and exchange trees must otherwise be disjoint; no other operator-profile content is granted read or list access.
