...
`Budget` is `{n_ctx,reserve,ceiling,used_est,used_measured,drift,cached_last:null|int,mode:exact|estimated,estimated,estimated_categories:[...],categories:{system,memory,tools,history,files,results,fetched,summary}}`.
...
- `server.probed` [3] `{server_id,capabilities,findings}`; `config.changed` [3] `{config,notice?}` (masked; `notice` is emitted for a one-time config migration); `error` [3] `{where,message}`.
